      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between h-16">
          ...
          <div className="hidden md:flex md:items-center">
            {calculatorCategories.map((category) => (
              <div key={category.name} className="relative group ml-8">
                <button
                  className="flex items-center text-gray-700 hover:text-brand-primary px-3 py-2 rounded-md text-sm font-medium"
                    ...
                    <div className="relative grid gap-2 bg-white p-4">
                      {category.links.map((link) => (
                        <Link
                          key={link.path}
                          to={link.path}
                          className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 hover:text-brand-primary rounded-md"
                        >
                          {link.name}
                        </Link>
                      ))}
            ...
            <a
              ...
              className="ml-8 inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md text-white bg-brand-primary hover:bg-brand-dark"
            >
          ...
          <div className="flex items-center md:hidden">
            <button
              onClick={toggleMenu}
              className="inline-flex items-center justify-center p-2 rounded-md text-gray-700 hover:text-brand-primary hover:bg-gray-100"
            >
          ...
          <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3">
            {calculatorCategories.map((category) => (
              <div key={category.name}>
                <button
                  className="w-full flex items-center justify-between px-3 py-2 rounded-md text-base font-medium text-gray-700 hover:text-brand-primary hover:bg-gray-100"
                  onClick={() => toggleCategory(category.name)}
                >
                  {category.name}
                  ...
                  <div className="pl-4 space-y-1">
                    {category.links.map((link) => (
                      <Link
                        key={link.path}
                        to={link.path}
                        className="block px-3 py-2 rounded-md text-base font-medium text-gray-700 hover:text-brand-primary hover:bg-gray-100"
            ...
            <a
              href="https://go.juliangoldie.com/strategy-session"
              className="block w-full text-center px-4 py-2 border border-transparent text-base font-medium rounded-md text-white bg-brand-primary hover:bg-brand-dark"
            >
              Book Free Strategy Call
            </a>
          </div>
